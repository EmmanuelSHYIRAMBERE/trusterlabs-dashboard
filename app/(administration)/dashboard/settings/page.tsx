...
import { SuccessMessage, UserProfile } from "@/types/profile";
...
export default function SettingsPage() {
  ...
  const { profile, setProfile, updateSessionProfile } = useProfileUpdates();
  ...
  const handleOpenProfileEdit = () => {
    setFormData({
      name: session?.user?.name || settings.name,
      email: session?.user?.email || settings.email,
      role: session?.user?.role || settings.role,
  ...
  /**
   * Handle profile photo upload with server action and real-time updates
   */
  const handlePhotoUpload = async (imageUrl: string) => {
    ...
    try {
      // Use server action for photo upload
      const result = await updateProfilePhoto(session.user.id, imageUrl);

      if (result.success) {
        // Update avatar in profile and session
        const updatedProfile = { ...profile!, avatarUrl: imageUrl };
        setProfile(updatedProfile);
        setEditedProfile(updatedProfile);

        // Update session for real-time sync
        await updateSessionProfile(updatedProfile);
                ...
                <div className="flex items-center gap-4">
                  {editedProfile?.avatarUrl ? (
                    <Image
                      ...
                      alt={editedProfile.fullName || "User Avatar"}
                      width={80}
                      height={80}
                    ...
                    <div className="w-20 h-20 bg-orange-500 rounded-full flex items-center justify-center text-white text-2xl font-semibold">
                      {editedProfile?.fullName?.charAt(0) ??
                  ...
                  <div className="flex-1">
                    <h3 className="text-lg font-semibold text-foreground">
                      {editedProfile?.fullName || session?.user?.name || "User"}
                    </h3>
                    <p className="text-sm text-muted-foreground">
                      {editedProfile.email || session?.user?.email}
                    </p>
                    <button
                      onClick={() => setIsUploadOpen(true)}
                      className="mt-2 px-3 py-1 text-sm bg-primary/20 text-primary rounded-lg hover:bg-primary/30 transition-colors"
                    >
                      Change Avatar
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-card rounded-lg border border-border">
                  <div>
                    ...
                    <p className="text-foreground font-medium">
                      {editedProfile.fullName || session?.user?.name}
                    </p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-2">
                  ...
                  <div>
                    ...
                    <p className="text-foreground font-medium">
                      {editedProfile.role || session?.user?.role}
                    </p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-2">
                ...
                {[
                  {
                    ...
                    description:
                      "Receive notifications for critical threats and vulnerabilities",
                  },
                  {
                    key: "weeklyReport",
                    label: "Weekly Security Reports",
                    description:
                      "Receive a summary of security events every week",
                  },
                  {
                  ...
                  },
                  {
                    ...
                    description:
                      "Receive updates on ongoing security incidents",
                  },
                ].map((item) => (
                  <motion.div
                    key={item.key}
              ...
              <div className="space-y-2">
                {[
                  ...
                  >
                    ...
                    <button className="text-sm text-red-critical hover:underline">
                      Logout
                    </button>
                  </div>
                ))}
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="flex items-center justify-between p-4 rounded-lg border border-border">
                    ...
                    <select
                      value={theme === "system" ? "auto" : (theme ?? "light")}
                      onChange={(e) =>
