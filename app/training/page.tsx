...
export default function TrainingPage() {
  const { items: programs, loading, error, apiCreate, apiUpdate, apiDelete } = useCRUD<TrainingProgram>('/api/training');
  ...
  const handleOpenCreate = () => {
    setFormData({ name: '', description: '', enrolled: 0, completed: 0, deadline: '', status: 'Active' });
    setErrors({});
    ...
    <DashboardLayout>
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-8">
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <StatCard label="Courses Completed" value={totalCompleted} change="+12 this month" isPositive={true} icon={<CheckCircle size={32} />} />
          <StatCard label="Active Programs" value={activeCount} change={`${programs.length} total`} icon={<Clock size={32} />} />
          <StatCard label="Total Enrolled" value={totalEnrolled} change="Across all programs" isPositive={true} icon={<BookOpen size={32} />} />
          <StatCard label="Completion Rate" value={`${completionRate}%`} change="+4% from last month" isPositive={true} icon={<Users size={32} />} />
        </motion.div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="h-3 bg-card rounded-full overflow-hidden">
                  <motion.div initial={{ width: 0 }} animate={{ width: `${completionRate}%` }} transition={{ duration: 1, delay: 0.3 }}
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="p-4 rounded-lg bg-green-success/10 border border-green-success/20">
                  ...
                  <p className="text-2xl font-bold text-green-success">{totalCompleted}</p>
                ...
                <div className="p-4 rounded-lg bg-orange-accent/10 border border-orange-accent/20">
                  ...
                  <p className="text-2xl font-bold text-orange-accent">{totalEnrolled - totalCompleted}</p>
                  <p className="text-xs text-muted-foreground mt-1">employees</p>
                </div>
                <div className="p-4 rounded-lg bg-teal-primary/10 border border-teal-primary/20">
                  ...
                  <p className="text-2xl font-bold text-teal-primary">{programs.length}</p>
        ...
        <motion.div variants={itemVariants}>
          ...
          {loading ? (
            <div className="py-12 text-center text-muted-foreground">Loading programs...</div>
          ) : (
            <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-6" variants={containerVariants}>
              {programs.map((program, idx) => (
                <motion.div key={program.id} variants={itemVariants}>
                  <Card className="h-full cursor-pointer hover:border-primary/50 transition-all"
                    title={program.name} subtitle={program.description}
                    action={<Badge variant={program.status === 'Active' ? 'success' : 'info'} size="md">{program.status}</Badge>}
                  >
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="h-2 bg-card rounded-full overflow-hidden">
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{ width: `${program.enrolled > 0 ? (program.completed / program.enrolled) * 100 : 0}%` }}
                      ...
                      <div className="flex gap-2 mt-4">
                        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleOpenEdit(program)}
                        ...
                        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setDeleteId(program.id)}
          ...
          <Card title="Upcoming Deadlines" subtitle="Critical training completion dates">
            <div className="space-y-3">
              {[...programs]
                ...
                .map((program, idx) => {
                  const daysUntil = Math.ceil((new Date(program.deadline).getTime() - Date.now()) / (1000 * 60 * 60 * 24));
                  return (
                    <motion.div key={program.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.05 }}
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground mt-1">Deadline: {new Date(program.deadline).toLocaleDateString()}</p>
                      </div>
                      <Badge variant={daysUntil <= 7 ? 'critical' : daysUntil <= 14 ? 'warning' : 'success'} size="md">
                        {daysUntil} days left
                      </Badge>
                    </motion.div>
                  );
              ...
              {programs.filter((p) => p.status === 'Active').length === 0 && (
                <p className="text-center text-muted-foreground py-8">No active programs</p>
              )}
            </div>
          </Card>
        </motion.div>
        ...
        <FormModal
          ...
          onClose={() => { closeModal('create-program'); closeModal('edit-program'); }}
          ...
          footer={
            <>
              <button onClick={() => { closeModal('create-program'); closeModal('edit-program'); }}
        ...
        >
          <div className="space-y-4">
            <FormInput label="Course Name" name="name" placeholder="e.g., Phishing Awareness 2025"
              value={formData.name || ''} onChange={(v) => setFormData({ ...formData, name: v })} error={errors.name} required />
            <FormTextarea label="Description" name="description" placeholder="Describe the training program and its objectives"
              value={formData.description || ''} onChange={(v) => setFormData({ ...formData, description: v })} error={errors.description} required rows={3} />
            <div className="grid grid-cols-2 gap-4">
              <FormInput label="Enrolled" name="enrolled" type="number"
                value={String(formData.enrolled ?? 0)} onChange={(v) => setFormData({ ...formData, enrolled: parseInt(v) || 0 })} error={errors.enrolled} required />
              <FormInput label="Completed" name="completed" type="number"
                value={String(formData.completed ?? 0)} onChange={(v) => setFormData({ ...formData, completed: parseInt(v) || 0 })} required />
            </div>
            <FormInput label="Deadline" name="deadline" type="date"
              value={formData.deadline || ''} onChange={(v) => setFormData({ ...formData, deadline: v })} error={errors.deadline} required />
          </div>
        </FormModal>

        <ConfirmDialog
          isOpen={deleteId !== null} title="Delete Training Program"
