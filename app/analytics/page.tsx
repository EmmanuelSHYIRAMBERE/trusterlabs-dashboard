      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <motion.div variants={itemVariants}>
            <Card title="Weekly Threat Activity" subtitle="Events detected vs resolved">
              <div className="h-80">
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={weeklyThreatData}>
                    ...
                    <Bar dataKey="resolved" fill="#4ade80" radius={[8, 8, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </Card>
          ...
          {/* Compliance Status */}
          <motion.div variants={itemVariants}>
            <Card title="Compliance Frameworks" subtitle="Audit score by framework">
                ...
                {complianceData.map((item, idx) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium text-foreground">{item.framework}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <motion.div variants={itemVariants}>
            <Card title="Threats by Department" subtitle="Incident distribution across teams">
              <div className="h-80">
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={threatByDepartment} layout="vertical">
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(139, 92, 246, 0.1)" />
                    <XAxis type="number" stroke="rgba(255, 255, 255, 0.4)" style={{ fontSize: '12px' }} />
                    <YAxis dataKey="department" type="category" stroke="rgba(255, 255, 255, 0.4)" style={{ fontSize: '12px' }} />
                    <Tooltip
                      contentStyle={{
                        backgroundColor: 'rgba(20, 20, 30, 0.9)',
                        border: '1px solid rgba(139, 92, 246, 0.3)',
                    ...
                    <Bar dataKey="resolved" fill="#4ade80" radius={[0, 8, 8, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </Card>
          ...
          <motion.div variants={itemVariants}>
            <Card title="Top Threat Categories" subtitle="Distribution of threat types">
              <div className="h-80">
                <PieChartComponent
                  data={topThreats}
        ...
        {/* Metrics Table */}
        <motion.div variants={itemVariants}>
          <Card title="Detailed Metrics" subtitle="Comprehensive statistics and trends">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    <th className="px-4 py-3 text-left font-semibold text-muted-foreground">Metric</th>
                    <th className="px-4 py-3 text-right font-semibold text-muted-foreground">Current</th>
                    <th className="px-4 py-3 text-right font-semibold text-muted-foreground">Last Week</th>
                    <th className="px-4 py-3 text-right font-semibold text-muted-foreground">Trend</th>
                  </tr>
                </thead>
                <tbody>
                  {[
                    { metric: 'Incidents Detected', current: 156, last: 142, trend: '+9.9%' },
                    { metric: 'Avg Resolution Time', current: '2.4h', last: '2.8h', trend: '-14.3%' },
                    { metric: 'False Positives', current: 23, last: 31, trend: '-25.8%' },
                    { metric: 'System Uptime', current: '99.98%', last: '99.95%', trend: '+0.03%' },
                    { metric: 'Vulnerabilities Found', current: 12, last: 15, trend: '-20.0%' },
                  ].map((row, idx) => (
                    <motion.tr
                      key={idx}
                      initial={{ opacity: 0 }}
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right font-semibold text-foreground">{row.current}</td>
                      <td className="px-4 py-3 text-right text-muted-foreground">{row.last}</td>
                      <td className={`px-4 py-3 text-right font-semibold ${row.trend.includes('-') ? 'text-green-success' : 'text-orange-accent'}`}>
                        {row.trend}
                      </td>
                    </motion.tr>
                  ))}
